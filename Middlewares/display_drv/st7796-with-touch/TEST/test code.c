...
void DrawTestPage(u8 *str)
{
	...
	//绘制固定栏down
	LCD_Fill(0,lcddev.height-20,lcddev.width,lcddev.height-1,BLUE);
	POINT_COLOR=GREEN;
...
void main_test(u8 *id)
{
	...
	POINT_COLOR=GREEN;
	Gui_StrCenter(0,132,"测试程序",32,1);
	POINT_COLOR=BLUE;
	Gui_StrCenter(0,170,id,24,1);
	POINT_COLOR=BLACK;
	Gui_StrCenter(0,196,"320RGB*480",24,1);
	POINT_COLOR=BLACK;
...
void FillRec_Test(void)
{
	...
	{
		POINT_COLOR=ColorTab[i];
		gui_fill_circle(80+(i*35),90,30,POINT_COLOR);
		delay_ms(100);
	}
	delay_ms(1500);
}
...
void Pic_test(void)
{
	...
	Gui_Drawbmp16(100,220,gImage_1);
	Gui_Drawbmp16(180,220,gImage_1);
	delay_ms(1500);
}
...
void Read_Test(void)
{
	...
	for (i=0; i<7; i++) 
	{
		...
		LCD_Fill(80-10,80+i*25-10,80+10,80+i*25+10,POINT_COLOR);
		color = LCD_ReadPoint(80,80+i*25);
		sprintf((char *)cbuf,"read color:0x%04x",color);
		Show_Str(80+20,80+i*25-8,cbuf,16,1);
	}
	delay_ms(1500);
}
