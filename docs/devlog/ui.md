...
LVGL 集成了外部字库，现在已经可以显示中文了，具体实现见 Middlewares/lvgl/fonts/
此外集成 LVGL 之后一定一定一定要把栈开得大大的，这玩意太容易爆栈了。

---

中文字库不知道什么原因炸掉了

UI 计划让 AI 生成大部分代码，自己只做架构上的设计和验证，我对 LVGL 不算特别熟悉（并且感觉学画 UI 似乎也没什么意义）

遇到的一个问题是 `lv_label_set_text` 和 `lv_label_set_text_fmt`, 这两个都是会触发 `realloc` 的，在文本更新频繁的场景下最好别，改用 `lv_label_set_text_static`， `realloc` 有可能因为内存碎片等原因炸掉（HardFault），且这玩意对 MCU 的开销也大

此外 LVGL 刷新文本时会有画面撕裂的问题，目前不知道为啥，也许真的要自己手搓个简单的 GUI 才能对 GUI 的工作原理有整体认知，现在完全就是在抓瞎，AI 又比较靠不住

---

因为暂时没办法解决局刷的问题，打算先回来折腾中文字库了

现象是在查找中文字库时使用了 `f_lseek`，然后有一定概率会在运行一段时间之后炸掉，我进入 DEBUG 之后发现这个炸掉的时间会慢一些，由此推测应该是时序问题。

经过一通 DEBUG 之后发现 fatfs 返回 `FR_DISK_ERR`，然后查阅文档得知应该是底层驱动问题，因此进入 `disk_read`，发现返回 `SDIO_Err_Generic`，此时我还是使用 `Read_Block` 模式的，还没开 DMA，之前想开但是出问题了就一直凑合用到现在。

然后进入 HAL 库驱动发现 `HAL_SD_ERROR_RX_OVERRUN` 被置位，询问 AI 得知是因为软件处理速度跟不上硬件，AI 提供了可能的排查方向：
- 中断响应延迟
- 任务调度影响
- CPU 处理不过来
- 硬件问题

由于 CPU 是 Cortex-M4 所以性能不可能太垃，三排除了，立创的板子且之前没出过问题，四也就排除了，我没用中断，自然中断也排除了，想到之前稍微调过任务优先级，于是把 UITask 的优先级改成所有任务中最高的，发现 OK 了，但是 UI 的优先级其实不应该那么高，那就只能考虑在 SDIO 读写的时候禁止调度了，所以要用到 `taskENTER_CRITICAL` 那两个配套函数，进临界区和出临界区，测试后发现是可以的。

**这也提醒了我们有些资源在操作的时候是不可以被打断的，以后写代码审代码时需要多多思考一下这方面的东西**

我觉得应该考虑下了解临界区的概念，然后再水一篇博客，此外参考文章：
https://news.eeworld.com.cn/mcu/article_2017022633932.html

要复现这个 BUG 的话，我想只要把现在加到 sdio_chip 里面所有的进临界区和出临界区的代码删掉，然后 UITask 任务维持原样（对中文字库的使用），再把优先级调低一些，应该就能复现了。

我想这里也许应该借助回调机制来实现进出临界区，毕竟 bsp 层也有可能不跑在 os 中

此外宏函数是真的一点都调试不了啊…别用宏函数了吧…，我还要特地把它改成正常函数才能断点进去调，md.
吃一堑吃一堑了属于是，看来得多思考一下这种多线程编程了，基础不牢地动山摇了属于是。
